...
CATEGORIES: Dict[str, List[str]] = {
    "comeback_campaign": [
        r"(?:#\s*)?comebackisreal\b",
        r"\bcome\s+back\s+is\s+real\b",
    ],
    ...
    "deposit_question": [
        r"how\s+(?:do\s+i\s+|to\s+)deposit",
        r"deposit\s+(?:method|option|way|how)",
        r"payment\s+method",
        r"how\s+(?:do\s+i\s+|to\s+)(?:top\s+up|topup|fund)",
        r"top\s*up\s+(?:method|how|account)",
        r"\bmin(?:imum)?\s+deposit\b",
        r"充值",
        r"存款",
        r"入金",
        r"怎么充",
    ...
    "withdrawal_question": [
        r"how\s+(?:do\s+i\s+|to\s+)withdraw",
        r"withdraw(?:al)?\s+(?:time|status|method|how|pending|stuck|failed|not\s+received|issue|problem)",
        r"when\s+(?:will\s+)?(?:i\s+)?(?:get|receive)\s+(?:my\s+)?(?:money|withdrawal|funds|winnings)",
        r"withdrawal\s+(?:not\s+arrived|missing|delayed)",
        r"\bmin(?:imum)?\s+withdrawal\b",
        r"提款",
        r"取款",
        r"出金",
        r"提现",
    ...
    "bonus_inquiry": [
        ...
        r"deposit\s+bonus",
        r"daily\s+bonus",
        r"loyalty\s+(?:point|reward|program|bonus)",
        r"how\s+(?:do\s+i\s+|to\s+)(?:get|claim|redeem)\s+(?:bonus|reward)",
        r"\bfree\s+(?:credit|spin|bet)\b",
        r"返水",
        r"红利",
        r"彩金",
        r"优惠活动",
    ...
    "game_question": [
        r"what\s+games?\s+(?:do\s+you\s+have|available|can\s+i\s+play)",
        r"how\s+to\s+play\s+\w+",
        r"best\s+(?:game|slot|table)\s+(?:to\s+play|here)",
        r"which\s+game\s+(?:is|has|gives)",
        r"game\s+(?:list|available|catalog)",
        r"any\s+game\s+recommendation",
        r"怎么玩",
        r"什么游戏",
        r"推荐游戏",
        r"好玩的游戏",
    ...
    "loss_share": [
        # Require a magnitude/amount qualifier to avoid matching "I lost my password/access"
        r"\bi\s+(?:just\s+)?lost\s+(?:big|everything|all|so\s+much|a\s+lot|\d)",
        r"\bmy\s+big\s+loss\b",
        r"\bkept?\s+losing\b",
        r"\bkeep\s+losing\b",
        r"\b(?:lost|losing)\s+(?:everything|all|so\s+much|a\s+lot|big)\b",
        r"\bbad\s+(?:luck|day|session|run|streak)\b",
        r"\b(?:so|very|really|super)\s+unlucky\b",
        r"\bunlucky\s+(?:today|again|streak)\b",
        r"\bnothing\s+(?:hit|came\s+in|worked)\b",
        r"输了",
        r"亏了",
        r"运气差",
        r"一直输",
